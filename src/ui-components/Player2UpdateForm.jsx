...
export default function Player2UpdateForm(props) {
  const {
    id,
    player2,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...player2Record };
    setUsername(cleanValues.username);
    setClicks(cleanValues.clicks);
    setErrors({});
  };
  const [player2Record, setPlayer2Record] = React.useState(player2);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Player2, id) : player2;
      setPlayer2Record(record);
    };
    queryData();
  }, [id, player2]);
  React.useEffect(resetStateValues, [player2Record]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Player2.copyOf(player2Record, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "Player2UpdateForm")}
    >
      <TextField
        label="Username"
        isRequired={true}
